...
atk_config.cli.add_cdh_command_line_options(parser)
args = parser.parse_args()

#get the cluster reference
cluster = atk_config.cdh.Cluster(args.host, args.port, args.username, args.password, args.cluster)

if cluster:
    #Read the generated configs

    configs = atk_config.get_auto_config()
    user_config = atk_config.aget_user_config()
    if user_config:
        #merge config dictionaries and resolve conflicts
        configs = atk_config.base.merge_dicts(user_config, configs)
    pprint(configs)
    ...
    user_conf = atk_config.atk.get_user_conf()
    if user_conf is None:
        #copy generated.conf to user.conf
        atk_config.atk.copy_generated_conf()
